...
printfn "%A" ((trajectory 2 2) |> Seq.take 5)

let between a b i = i < (max a b) && i > (min a b)

let maxy minxc maxxc minyc maxyc =
    seq {1..maxxc} 
    |> Seq.allPairs (seq {0..1000})
    |> Seq.map (fun (x, y) -> trajectory x y)
    |> Seq.map (fun traj -> Seq.takeWhile (fun (x, y) -> x < maxxc + 1 && y > (min maxyc minyc) + 1) traj)
    |> Seq.filter (fun traj -> Seq.exists (fun (x, y) -> (between minxc maxxc x) && (between maxyc minyc y)) traj)
    |> Seq.map (fun traj -> Seq.maxBy (fun (_, y) ->  y) traj)
    |> Seq.map (fun (x, y) -> y)
    |> Seq.max

printfn "%A" (maxy 20 30 -10 -5)